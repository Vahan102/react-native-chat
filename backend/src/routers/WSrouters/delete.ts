...
export async function deleteWBSRouter(msg: any, ws: WebSocket, users: any) {
    try {
        const user = await verificationToken(msg.token);
        if (user != false) {
            users.set(user.email, ws);
            if (msg.type == "deletedMsg") await deleteMsg(msg, ws, users)
        } else ws.terminate();
    } catch (err) {
        throw err
    }
...
async function deleteMsg(msg: any, ws: WebSocket, users: Map<any, any>) {
    try {
        let user;
